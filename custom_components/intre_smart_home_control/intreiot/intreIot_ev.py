...
from .intreIot_error import IntreIoTEvError
...
class IntreIoTEventLoop:
    """IntreIoT event loop."""
    _poll_fd: selectors.DefaultSelector

    _fd_handlers: dict[str, IntreIoTFdHandler]

    _timer_heap: list[IntreIoTTimeout]
    _timer_handlers: dict[str, IntreIoTTimeout]
    ...
    def loop_forever(self) -> None:
        ...
        while True:
            ...
            while len(self._timer_heap) > 0:
                timer: IntreIoTTimeout = self._timer_heap[0]
                if timer is None:
                    break
            ...
            for key, mask in events:
                fd_handler: IntreIoTFdHandler = key.data
    ...
    def set_timeout(
        ...
        if timeout_ms is None or handler is None:
            raise IntreIoTEvError('invalid params')
        new_timeout: IntreIoTTimeout = IntreIoTTimeout()
    ...
    def clear_timeout(self, timer_key: TimeoutHandle) -> None:
        ...
        if timer_key is None:
            return
        timer: IntreIoTTimeout = self._timer_handlers.pop(timer_key, None)
        if timer:
    ...
    def __set_handler(
        ...
        if not self._poll_fd:
            raise IntreIoTEvError('event loop not started')

        fd_key: str = str(id(fd))
        fd_handler = self._fd_handlers.get(fd_key, None)

        if fd_handler is None:
            fd_handler = IntreIoTFdHandler(fd=fd)
            fd_handler.fd = fd
