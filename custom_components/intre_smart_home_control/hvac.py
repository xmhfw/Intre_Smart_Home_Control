...
async def async_setup_entry(
    ...
    for hadevice in _hadevices:
        ...
        for entity in entitys:
            #_LOGGER.debug('hvac create'+entity['entry'].entity_id)
            name = entity.name.lower() if entity.name else ""
            entity_id = entity.entity_id.lower()
            attributes = entity.attributes
            if entity['entry'].entity_id.split(".")[0]== 'climate':
                module_info={}
                ...
                floorHeating :IntreHavc = IntreHavc(intre_ss=intre_ss,product=product,module_info=module_info)
                product.add_modules(floorHeating)
class IntreHavc(IntreIoTModule):
    ...
    def get_module_json(self)->dict:
        ...
        if match:
            index = match.group(1)
            instance_module_name = f"HAVC{index}"
        else:
            # 匹配失败时使用默认名称
