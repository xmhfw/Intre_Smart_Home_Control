    ...
    for hadevice in _hadevices:
        ...
        for entity in entitys:
            ...
            if entity_id.split(".")[0]== 'light':
                ...
                _LOGGER.debug("RGBCWLight light.")
                _LOGGER.debug(state)
                if state is not None:
                    attributes = state.attributes
                    supported_color_modes = attributes.get('supported_color_modes', 'N/A')
                    if 'rgbww_color' in supported_color_modes:# 判断是否RGBCWLight
                
                        module_info={}
                        _LOGGER.debug('RGBCWLight create')
                        ...
                        light :IntreRGBCWLight = IntreRGBCWLight(hass=hass,intre_ss=intre_ss,product=product,module_info=module_info)
                        product.add_modules(light)


class IntreRGBCWLight(IntreIoTModule):
    _product:IntreIoTProduct
    _intre_ss:IntreManagementEngine
    min_color_temp_kelvin:str
    max_color_temp_kelvin:str
    _hass:HomeAssistant
    def __init__(self,hass,intre_ss:IntreManagementEngine,product:IntreIoTProduct,module_info:dict) -> None:
        ...
        max_color_temp_kelvin = str(state.attributes.get('max_color_temp_kelvin', 'N/A'))
        min_color_temp_kelvin = str(state.attributes.get('min_color_temp_kelvin', 'N/A'))
    ...
    def get_module_json(self)->dict:
        ...
        if match:
            index = match.group(1)
            instance_module_name = f"RGBCW{index}"
        else:
            # 匹配失败时使用默认名称
    ...
    async def _entity_state_notify(self,newstate)->None:
        ...
        _LOGGER.debug(
            f"Brightness: {brightness}, rgb: {rgb},Color Temp: {color_temp}"    
        )
...
async def test_fun()->bool:
    _LOGGER.debug("test-RGBCE_light")  
